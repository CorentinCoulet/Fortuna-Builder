...
function changeLanguage(selectedLanguage) {
    ...
    i18n.changeLanguage(currentLanguage, (err, t) => {
        if (err) return console.log('Erreur lors du changement de langue', err);
        document.querySelector('.example-element').textContent = t('exampleKey');
    });
}
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    for (const dropdown of content.children) {
        dropdown.addEventListener("click", function(){
            ...
            var selectedLanguage = dropdown.getAttribute("data-lang");
